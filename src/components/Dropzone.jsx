...
function MyDropzone(props) {
  const {setImageVacante} = useVacante();
  ...
  const SendImagetoServer = async (file) => {
    ...
    try {
      const response = await clientAxios.post('/api/image', formData, {
          headers: {
      ...
      setImageVacante(response.data.imagen); 
    } catch (error) {
     console.log(error);
    }
  }
 ...
 const files = acceptedFiles.map(file => (
  ...
  if (hasError) {
    Swal.fire({
      title: 'Lo sentimos, solo se permite subir una imagen por Vacante',
  ...
  return (
    <section className="container bg-gray-200   w-full  p-5 rounded-md">
      <div {...getRootProps({className: 'dropzone'})} className='bg-gray-300 hover:bg-gray-500 cursor-pointer transition-all hover:-translate-y-2 mb-4 p-2 rounded-md'>
        <input {...getInputProps()} />
      ...
      <aside className='bg-gray-500 p-2 rounded-md'>
